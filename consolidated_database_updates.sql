-- =====================================================
-- CONSOLIDATED DATABASE UPDATES FOR FINANZAS MODULE
-- =====================================================
-- Execute this script in Supabase SQL Editor to apply all changes

-- 1. Add origen column to movimientos_financieros
DO $$ 
BEGIN
    IF NOT EXISTS (
...
-- 2. Add color column to subcategorias_financieras
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
...
-- 3. Create movimientos_recurrentes table if it doesn't exist
CREATE TABLE IF NOT EXISTS public.movimientos_recurrentes (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    usuario_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
    tipo TEXT NOT NULL CHECK (tipo IN ('Ingreso', 'Egreso')),
    categoria_id UUID REFERENCES public.categorias_financieras(id) ON DELETE RESTRICT,
    subcategoria_id UUID REFERENCES public.subcategorias_financieras(id) ON DELETE RESTRICT,
    ...
    dia_de_la_semana INTEGER CHECK (dia_de_la_semana >= 1 AND dia_de_la_semana <= 7),
...
-- 4. Add foreign key constraints (with error handling)
DO $$
BEGIN
    -- Add categoria foreign key if it doesn't exist
    IF NOT EXISTS (
    ...
    -- Add subcategoria foreign key if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
...
-- 7. Create RLS policies (with error handling)
DO $$
BEGIN
