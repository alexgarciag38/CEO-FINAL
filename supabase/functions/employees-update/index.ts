...
type Body = { id: string; name?: string; email?: string; is_active?: boolean };
...
serve(async (req) => {
  ...
  try {
    ...
    const { data, error: err } = await supabase.from('employees').update(updates).eq('id', body.id).select('*').single();
    if (err) throw err;
    return ok({ item: data });
  } catch (e) {
    return fail(e);
