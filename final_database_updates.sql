-- =====================================================
-- FINAL DATABASE UPDATES FOR FINANZAS MODULE
-- =====================================================
...
-- Add check constraint for origen values (with error handling)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'check_origen_values'
...
-- 4. Add foreign key constraints (separate statements to avoid errors)
-- First, add categoria foreign key
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
...
-- Second, add subcategoria foreign key
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
...
-- 7. Create RLS policies (drop and recreate to avoid conflicts)
