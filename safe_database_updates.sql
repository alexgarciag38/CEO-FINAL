-- =====================================================
-- SAFE DATABASE UPDATES FOR FINANZAS MODULE
-- =====================================================
...
-- Add check constraint for origen values (only if column exists)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'movimientos_financieros' AND column_name = 'origen' AND table_schema = 'public') THEN
        BEGIN
            ALTER TABLE public.movimientos_financieros 
            ADD CONSTRAINT check_origen_values 
            CHECK (origen IN ('unico', 'recurrente'));
        EXCEPTION
...
-- STEP 4: Add check constraints to movimientos_recurrentes (with error handling)
DO $$
BEGIN
    -- Add tipo constraint
    BEGIN
    ...
    -- Add frecuencia constraint
    BEGIN
        ALTER TABLE public.movimientos_recurrentes 
        ADD CONSTRAINT check_frecuencia_values 
        CHECK (frecuencia IN ('Diaria', 'Semanal', 'Quincenal', 'Mensual', 'Bimestral', 'Trimestral', 'Semestral', 'Anual'));
    EXCEPTION
        WHEN duplicate_object THEN
            -- Constraint already exists, ignore
            NULL;
    ...
    -- Add dia_del_mes constraint (only if column exists)
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'movimientos_recurrentes' AND column_name = 'dia_del_mes' AND table_schema = 'public') THEN
        BEGIN
            ALTER TABLE public.movimientos_recurrentes 
            ADD CONSTRAINT check_dia_del_mes 
            CHECK (dia_del_mes >= 1 AND dia_del_mes <= 31);
        EXCEPTION
            WHEN duplicate_object THEN
                -- Constraint already exists, ignore
                NULL;
    ...
    -- Add dia_de_la_semana constraint (only if column exists)
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'movimientos_recurrentes' AND column_name = 'dia_de_la_semana' AND table_schema = 'public') THEN
        BEGIN
            ALTER TABLE public.movimientos_recurrentes 
            ADD CONSTRAINT check_dia_de_la_semana 
            CHECK (dia_de_la_semana >= 1 AND dia_de_la_semana <= 7);
        EXCEPTION
            WHEN duplicate_object THEN
                -- Constraint already exists, ignore
                NULL;
...
-- STEP 8: Create RLS policies (drop and recreate to avoid conflicts)
DROP POLICY IF EXISTS "Users can view their own recurring movements" ON public.movimientos_recurrentes;
DROP POLICY IF EXISTS "Users can insert their own recurring movements" ON public.movimientos_recurrentes;
DROP POLICY IF EXISTS "Users can update their own recurring movements" ON public.movimientos_recurrentes;
DROP POLICY IF EXISTS "Users can delete their own recurring movements" ON public.movimientos_recurrentes;
